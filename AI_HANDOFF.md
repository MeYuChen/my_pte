...
5. 新增 `刷卡` 模式，把 39 篇拆成中文路线、英文关键词、4 句骨架、混合提取四类卡片，用户用“忘了 / 模糊 / 记住了”记录提取熟练度。
...
`刷卡` 模式面向手机碎片时间使用。每篇文章按 `DRILL_TYPES` 生成 4 张卡：

- `route`：看到题目，回忆中文路线和中文钩子。
- `keywords`：根据中文钩子，回忆 5 个英文关键词。
- `skeleton`：根据关键词，默写或口述 4 句英文骨架。
- `mixed`：看到题目，整条链路一次性提取。

评分只保存在浏览器 `localStorage` 的 `persisted.drill`，当前不做账号、云同步和间隔重复调度。修改刷卡体验时，优先保证手机单卡流程不痛苦：问题清楚、答案可一键展开、底部只有“忘了 / 模糊 / 记住了”三种结果。
